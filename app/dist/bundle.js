...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 * & RXjs
	 */
	var $wrapper = document.querySelector('#range');
	...
	document.querySelector('#default').addEventListener('change', function (evt) {
	  console.log(evt.target.value);
	});

	var activateHandle = function activateHandle() {
	  return $handle.classList.add('active');
	};
	var deactivateHandle = function deactivateHandle() {
	  return $handle.classList.remove('active');
	};
	 ...
	 * @type {ClientRect}
	 */
	var wrapper = $wrapper.getBoundingClientRect();
	var handle = $handle.getBoundingClientRect();
	 ...
	 * we would like
	 */
	var range = (0, _range2.default)(wrapper, handle, 5);
	 ...
	 */
	var handle$ = new _rx2.default.BehaviorSubject(range.handleOffset(initial));

	/**
	 * Now because we have the BehaviorSubject, we can subscribe to it and
	 * update the DOM element appropriately
	 */
	handle$.subscribe(function (x) {
	  return $handle.style.left = x + 'px';
	});
	 ...
	 */
	var MOUSE = {
	  down: _rx2.default.Observable.fromEvent($wrapper, 'mousedown'),
	  move: _rx2.default.Observable.fromEvent(document, 'mousemove'),
	  up: _rx2.default.Observable.fromEvent(document, 'mouseup')
	};
	 ...
	 */
	var drag$ = MOUSE.down.do().flatMap(function (md) {
	  return MOUSE.move.pluck('clientX').startWith(md.clientX).takeUntil(MOUSE.up);
	})
	/**
	 ...
	 * subject.
	 */
	drag$.map(function (left) {
	  return range.handlePositionFromOffset(left);
	}).subscribe(handle$);
	 ...
	 */
	MOUSE.down.flatMap(MOUSE.up.take(1)).withLatestFrom(drag$, function (_, left) {
	  return range.stepFromOffset(left);
	}).do(function (x) {
	  return console.log('Value emitted', x);
	}).subscribe();
...
/***/ function(module, exports) {
	 ...
	 */
	function create(wrapper, handle, steps) {

	    var maxStep = steps - 1;
	    var segmentSize = (wrapper.width - handle.width) / maxStep;

	    /**
	     * @param step
	     * @returns {number}
	     */
	    var handleOffset = function handleOffset(step) {
	     ...
	     * @returns {number}
	     */
	    var stepFromOffset = function stepFromOffset(offset) {
	        var nearest = Math.round((offset - handle.width / 2) / segmentSize);
	    ...
	    return {
	        ...
	        handlePositionFromOffset: function handlePositionFromOffset(offset) {
	            return handleOffset(stepFromOffset(offset));
	        }
	    };
	}
